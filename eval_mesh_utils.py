...
# Modified for [DB-TSDF] by José E. Maese.

import open3d as o3d
import numpy as np
import time

def eval_pcd(file_pred, file_trgt, down_sample_res=0.05, threshold=0.2, truncation_acc=0.50, truncation_com=0.50, gt_bbx_mask_on= True, 
    ...
    pcd_sample_pred = o3d.io.read_point_cloud(file_pred) 
    ######### ICP #############
    # pcd_trgt_icp = o3d.io.read_point_cloud("gt/ncd_quad_gt.ply")
    pcd_trgt_icp = o3d.io.read_point_cloud("gt/gt_map_pc_mai.ply")
    pcd_pred, T_icp = apply_icp_multiscale(pcd_sample_pred, pcd_trgt_icp, seed=42)
    # pcd_pred = pcd_sample_pred
    # pcd_pred = apply_icp(pcd_pred_raw=pcd_sample_pred, 
    #                      pcd_trgt_raw=pcd_trgt,
    #                     #  pcd_trgt_raw=pcd_trgt_icp, 
    #                      voxel_icp=0.01, 
    #                      icp_thresh=0.10, 
    #                      target_pts=500_000, 
    #                      seed=42)
    ...
    if gt_bbx_mask_on: 
        ...
        pcd_pred = pcd_pred.crop(trgt_bbx)
    ...
    d = show_gt_pred_colored(pcd_pred, pcd_trgt, vmax=0.20)
    print(" % puntos < 5cm:", np.mean(d < 0.05)*100)
    print(" % puntos <20cm:", np.mean(d < 0.20)*100)
...
# ---- helpers rápidos -------------------------------------------------
def _centroid_yaw_init(src: o3d.geometry.PointCloud, dst: o3d.geometry.PointCloud):
    """Inicializa con traslación de centroides + yaw por PCA en XY (rápido y robusto)."""
    Ps = np.asarray(src.points); Pd = np.asarray(dst.points)
    cs = Ps.mean(0); cd = Pd.mean(0)

    # PCA en XY para estimar yaw
    def _yaw(P):
        Q = P[:, :2] - P[:, :2].mean(0)
        C = (Q.T @ Q) / max(len(Q), 1)
    ...
    T = np.eye(4)
    T[:3, :3] = Rz
    T[:3,  3] = cd - Rz @ cs  # rota alrededor del origen y traslada al centro destino
    return T

def _prep(pc, vox, need_normals):
    q = pc.voxel_down_sample(vox)
    if need_normals:
        q.estimate_normals(o3d.geometry.KDTreeSearchParamHybrid(radius=3*vox, max_nn=30))
    return q

def _icp_once(src, dst, th, mode, max_it):
    if mode == "p2p":
    ...
    crit = o3d.pipelines.registration.ICPConvergenceCriteria(
        max_iteration=max_it, relative_fitness=1e-7, relative_rmse=1e-7)

    reg = o3d.pipelines.registration.registration_icp(
        src, dst, th, np.eye(4), est, crit)

    print(f"   ICP {mode:>3} @ voxel={th/3:.2f} → fitness={reg.fitness:.4f}, rmse={reg.inlier_rmse:.4f}")
    return reg.transformation


# ---- ICP multiescala rápido ------------------------------------------
def apply_icp_multiscale(pcd_pred_raw, pcd_trgt_raw, seed=42):
    t0 = time.time()

    # 1) Muestras grandes pero acotadas (evita OOM y acelera)
    def rs(pc, n):
        if len(pc.points) <= n: return pc
        rng = np.random.default_rng(seed)
        idx = rng.choice(len(pc.points), n, replace=False)
        return pc.select_by_index(idx)

    pred = rs(pcd_pred_raw, 2_000_000)
    trgt = rs(pcd_trgt_raw,  2_000_000)

    # 2) Inicialización rápida sin RANSAC
    T = _centroid_yaw_init(pred, trgt)
    pred.transform(T)

    # 3) Pirámide (pensada para 70×45×30 m; GT 1 cm, mapa 5 cm)
    #    (voxel,  umbral,  modo,  iter)
    levels = [
        (0.40, 1.20, "p2p", 150),
        (0.20, 0.60, "p2p", 150),
        (0.10, 0.30, "p2l", 200),
    ...
    for vox, th, mode, iters in levels:
        sp = _prep(pred, vox, need_normals=(mode == "p2l"))
        tg = _prep(trgt, vox, need_normals=(mode == "p2l"))
        dT = _icp_once(sp, tg, th, mode, iters)
        pred.transform(dT)
        T = dT @ T

    print(f"ICP multiescala OK · tiempo = {time.time()-t0:.1f}s")
    return pred, T
...
def color_map_jet01(x):
    """x in [0,1] → RGB jet-like."""
    x = np.clip(x, 0.0, 1.0)
...
def nn_distances(src_pts, ref_pts):
    ref = o3d.geometry.PointCloud()
    ref.points = o3d.utility.Vector3dVector(ref_pts.astype(np.float64))
    kdt = o3d.geometry.KDTreeFlann(ref)
    d = np.empty(len(src_pts), dtype=np.float32)
    for i, v in enumerate(src_pts):
        _, _, dsq = kdt.search_knn_vector_3d(v, 1)
        d[i] = np.sqrt(dsq[0])
    return d

def show_gt_pred_colored(pcd_pred, pcd_trgt, vmax=0.20):
    """Colorea pcd_pred por error a GT y muestra ambos."""
    pred_pts = np.asarray(pcd_pred.points)
    trgt_pts = np.asarray(pcd_trgt.points)
    d = nn_distances(pred_pts, trgt_pts)   

    # normalizar y mapear a color
    x = np.clip(d / float(vmax), 0.0, 1.0)
    colors = color_map_jet01(x)
    pcd_pred.colors = o3d.utility.Vector3dVector(colors)

    # GT en gris
    pcd_trgt_col = o3d.geometry.PointCloud(pcd_trgt)
    pcd_trgt_col.paint_uniform_color([0.6, 0.6, 0.6])

    print(f"[Viz] ACC pred→GT  min/med/max = {d.min():.3f}/{np.median(d):.3f}/{d.max():.3f} m  (vmax={vmax} m)")
    o3d.visualization.draw_geometries([pcd_trgt_col, pcd_pred])

    return d  

if __name__ == "__main__":
    from pathlib import Path
    # pred_file = Path("college/vdb_gpdf_mesh.ply")
    # gt_file   = Path("gt/ncd_quad_gt.ply")
    ...
    pred_file = Path("college/mesh.ply")
    gt_file   = Path("gt/gt_map_pc_mai.ply")
